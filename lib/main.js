...
module.exports = (config) => {
  ...
  const consoleTypes = config.methods;
  ...
  consoleTypes.forEach((method) => {
    ...
    console[method] = function () {
      ...
      let path = firstStack.split(':')[0].split('(').length > 2 ? firstStack.split(':')[0].split('(')[2] : firstStack.split(':')[0].split('(')[1];
      path = path ? path : firstStack.split(':')[0].split('at')[1];
